...
mrxmlgen_loc = "../SteerSuitePrivate/testcases/Merseyrail"
mrxmlgen_name = "GenerateSetup.py"

# Steersuite
ss_wd = "../SteerSuitePrivate/build/vs2012/"
ss_exe = "../bin/steersim.exe"
ss_config = "TimingConfig.xml"
...
sd_arr = [0, 1.0,  2.0]
num_arr = np.arange(5, 65, 5)
nc_arr = np.arange(5, 65, 5)
						...
						for sd in sd_arr:
							...
							for n in num_arr:
								for nc in nc_arr:
									...
									try:
										subprocess.check_output(["python", mrxmlgen_name, "-n", str(n), "-nc", str(nc), "-r", str(r), "-d", str(d), "-c", str(car)], stderr=subprocess.PIPE)
									...
									try:
										...
										ss_output = subprocess.check_output((ss_exe, "-config", ss_config, "-commandline"), stderr=subprocess.STDOUT)
										# print("done")
									except subprocess.CalledProcessError as e:
										os.chdir(path)
										num_failed +=1
